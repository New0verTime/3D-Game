...
void Object::RenderObject(){
    if(inside&&delta_angle-epsilon<60&&dist>lim){
        ...
        if( (int((delta_angle+i)*10)>=0&&int((delta_angle+i)*10)<=600)&&(((x+obj_Width*std::sin(epsilon)-TheRay_cast::Instance()->get_rayinfox(int((delta_angle+i)*10)) )*(x+obj_Width*std::sin(epsilon)-ThePlayer::Instance()->getPosx()/50) <0 )
           || ( (y+obj_Width*std::cos(epsilon)-TheRay_cast::Instance()->get_rayinfoy(int((delta_angle+i)*10)) )*(y+obj_Width*std::cos(epsilon)-ThePlayer::Instance()->getPosy()/50) <0)) ){
